...
cc     real scf
c
cc Note that this is very crude for many reasons, and should be
cc   improved.  See below for an alternative solution.
c
c      albedo_veg = 0.15
c
cc Define the albedo and roughness length.
c      if (snow_d.gt.0.0) then
cc Snow.
c        z_0 = snow_z0
c        if (Tair.gt.Tf) then
c          if (vegtype.le.5.0) then
cc Melting.  Assume, because of leaf litter, etc., that the snow
cc   albedo under a forest canopy is different than in a clearing.
c            albedo = albedo_snow_forest
c          else
c            albedo = albedo_snow_clearing
c          endif
cc For thin melting snowcovers (less than 15 cm), reduce the albedo
cc   to account for the observed enhanced melting processes.
cc         scf = min(1.0,snow_d/0.15)
cc         albedo = scf * albedo + (1.0 - scf) * albedo_veg
c        else
cc Dry.
c          albedo = 0.8
c        endif
...
cc No snow.
c      else
c        if (vegtype.eq.20.0) then
cc Glacier or permanant snow.
c          z_0 = snow_z0
c          albedo = albedo_glacier
c        else
cc Land.
c          z_0 = veg_z0_tmp
c          if (vegtype.le.5.0) then
c            z_0 = 0.2
c          elseif (vegtype.le.11.0) then
c            z_0 = 0.04
c          else
c            z_0 = 0.02
c          endif
